...
export function createScene() {
    ...
    function initialize() {
        ...
        //         .load("C:/TermProject/Imsi_Map_007/Model/hdri_background_image", (texture) => {
        ...
        // scene.background = this.setupBackground();                       
    ...
    const infoList = document.getElementById('info-panel');
    if (!infoList || newCityInfo.length === 0) {
        console.error('Error: Unable to find the element with id "info-panel" or newCityInfo is empty.');
        return;
    }
    ...
    //     infoElement.innerHTML = `<strong>${newCity.name}</strong>: 인구 - ${newCity.population}`;
