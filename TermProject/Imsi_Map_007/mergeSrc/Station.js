...
export function createStation(scene, x, y, StationList) {
  ...
  modelPaths.forEach((modelPath, index) => {
    loader.load(
      ...
      (gltf) => {
        ...
        model.position.set(roundX, 23, roundY);
        models.push(model);
        scene.add(model);
  ...
  updateStationInfoHTML(StationList);
...
// 역 info를 html로 변환
export function updateStationInfoHTML(StationList) {
  const stationInfoDiv = document.getElementById("station-info");
  stationInfoDiv.innerHTML = ""; // Clear previous information

  // Create a wrapper div to contain station information
  const stationInfoWrapper = document.createElement("div");

  // Add the static "station info" section
  const stationInfoHeader = document.createElement("div");
  stationInfoHeader.innerHTML = `
        <div style="font-weight: bold;">Station Information:</div>
    `;
  stationInfoWrapper.appendChild(stationInfoHeader);

  stationInfoDiv.appendChild(stationInfoWrapper);

  for (const StationInfo of StationList) {
    const stationHTML = document.createElement("div");
    stationHTML.innerHTML = `
            <div>Station${StationInfo.mark}, X: ${StationInfo.x}, Y: ${StationInfo.y}</div>
        `;
    stationInfoWrapper.appendChild(stationHTML);
  }

  // function updatePeopleCount(x, y, newPeopleCount) {
  //     // Find the station in StationList by its coordinates (x, y)
  //     const stationToUpdate = StationList.find(station => station.x === x && station.y === y);

  //     if (stationToUpdate) {
  //         // Update the people count for the found station
  //         stationToUpdate.people = newPeopleCount;
  //     }
  // }

  // Function to handle click events and update the people count
  stationInfoDiv.addEventListener("click", (event) => {
    const clickedElement = event.target;
    if (clickedElement.tagName === "DIV") {
      // Assuming the last child div is the 'People' information
      const stationDiv = clickedElement.parentNode;
      const x = parseInt(stationDiv.children[0].innerText.split(" ")[1], 10);
      const y = parseInt(stationDiv.children[1].innerText.split(" ")[1], 10);
      // updatePeopleCount(x, y, StationList.find(station => station.x === x && station.y === y).people + 1);
      updateStationInfoHTML(StationList); // Update the rendered HTML again
    }
  });
}
